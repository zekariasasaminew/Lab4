...
def drawOneDonut(window, glazeColor, backgroundColor, radius, center):
    ...
    drawSprinkles(window, mainCircle)
...
def drawSprinkles(window, donut):
    ...
    """
    center = donut.getCenter()
    
    radius = donut.getRadius()
    for i in range(NUM_SPRINKLES):
        x1= center.getX()
        y1 = center.getY()
        x = random.randrange(x1-radius,x1+radius)
        y = random.randrange(y1-radius,y1+radius)
        color = random.choice(COLOR_LIST)
        
        distance = math.sqrt((x1-x)**2+(y1-y)**2)
        if distance>(radius*HOLE_RATIO) and distance < radius - SPRINKLE_RADIUS:
            sprinkles = Circle(Point(x,y),SPRINKLE_RADIUS)
            sprinkles.setFill(color)
            sprinkles.draw(window)
